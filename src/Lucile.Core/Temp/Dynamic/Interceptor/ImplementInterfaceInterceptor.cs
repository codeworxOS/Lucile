...
{
    public class ImplementInterfaceInterceptor<T> : ImplementInterfaceInterceptor where T : class
    {
    ...
    public class ImplementInterfaceInterceptor : ITypeDeclarationInterceptor
    {
        ...
        public ImplementInterfaceInterceptor(Type interfaceType)
        {
            ...
            if (!this.InterfaceType.IsInterface) {
                throw new ArgumentOutOfRangeException("interfaceType", "The type parameter must be an interface.");
            }
        }
            ...
            foreach (var item in this.InterfaceType.GetMethods()) {
                if (!IsExplicitImplemented(config, builder, item)) {
                    ...
                    if (!interfaceAccessorMethods.Contains(item)) {
                        ...
                        if (method == null) {
#if(SILVERLIGHT)
                throw new MissingMemberException(string.Format("Missing Member {1} on type {0}",builder.Name,item.Name));
#else
